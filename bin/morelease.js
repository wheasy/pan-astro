...
var release = {
    dealImageNew: function(projectName, type) {
        ...
        var pagesHash,
            releaseDir = pan.getFilePath(projectName, 'release'),
            releaseImgDir = path.join(releaseDir, 'img', type);

        if (type == 'p') {
            pagesHash = mo.swap('pagesHash') || {};
        } else if (type == 'm') {
            pagesHash = mo.swap('modulesHash') || {};
        } else {
            pagesHash = mo.swap('mojsHash') || {};
        }
    ...
    dealImage: function(projectName, type) {
        if (!mutil.in_array(type, M_TYPE)) return;
        var releaseDir = pan.getFilePath(projectName, 'release'),
            releaseImgDir = path.join(releaseDir, 'img', type),
        ...
        if (type == 'p') {
            dealDir = pan.getFilePath(projectName, 'pages'),
                reg = reg_pages;
        } else if (type == 'm') {
            dealDir = pan.getFilePath(projectName, 'modules'),
                reg = reg_modules;
        } else {
            dealDir = pan.getFilePath(projectName, 'jslib'),
                reg = reg_mojs;
        }
    ...
    dealPageSimple: function(projectName) {
        ...
        compiler.loadModule(projectName);
        compiler.loadLibJS(projectName);
        var pagesHash = mo.swap('pagesHash') || {},
            releaseDir = pan.getFilePath(projectName, 'release'),
    ...
    dealRoView: function(projectName) {
        ...
        if (!mutil.isEmptyObject(pagesHash[projectName])) {
            mutil.each(pagesHash[projectName], function(page, index) {
                ...
                compress.parse_less(projectName, pageName, true, function(css, time) {
                    if (time) {
                        ...
                        cssName = cssName + '.css',
                            cssfile = path.join(cssDir, cssName);
                        var cssPage = path.join(cssDir,  pageName , 'app.css');
                        //修改json文件
                        var pageObj = !mutil.isEmptyObject(jsonConf[pageName]) ? jsonConf[pageName] : {};
                        /*if(mutil.isEmptyObject(pageObj)||pageObj['css']!=cssName){//判断page对象是否存在 不存在或与上一次文件不一致
                            if(isMd5){
                ...
                compress.parse_js({
                    ...
                    callback: function(js, time) {
                        if (time) {
                            ...
                            var jsPage = path.join(jsDir, pageName , 'app.js');
                            //修改json文件
                            jsonConf = jsonConf ? jsonConf : {};
                            var pageObj = !mutil.isEmptyObject(jsonConf[pageName]) ? jsonConf[pageName] : {};
                            /*if(mutil.isEmptyObject(pageObj)||pageObj['js']!=jsName){//判断page对象是否存在 不存在或者上次修改时间小于修改时间
            ...
            //写入json配置
            fs.writeFileSync(confJsonFile, jsonConf, 'utf8'); //生成文件*/
            var seedjs = moto.getMotoSeedJs(projectName);
    ...
    dealCommonImg: function(projectName) {
        var releaseDir = pan.getFilePath(projectName, 'release'),
            imgDir = pan.getFilePath(projectName, 'img'),
    ...
    dealCommonJs: function(projectName) {
        var releaseDir = pan.getFilePath(projectName, 'release'),
            ...
            jsDir = pan.getFilePath(projectName, 'js'),
            jsonDir = pan.getFilePath(projectName, 'json'),
            confJsonFile = path.join(jsonDir, 'conf.json'),
            jsonConf = {},
            isMd5 = 0; //打开MD5文件
    ...
    //type=single,common
    releaseProject: function(projectName, cfg) {
        cfg = cfg || {};
        var type = cfg.type;
        if (!mutil.in_array(type, ['common', 'single'])){
            console.log('请指定项目类型!');
            return;  
        } 
        if (type) {
            //合并雪碧图
            img.sprite.merge(projectName);
            switch (type) {
                case "single":
